...
const defaultTask = { title: "", description: "", status: "En attente" };
...
const ProjectMembersTasksManager = () => {
  ...
  // MEMBERS & TASKS
  const [members, setMembers] = useState([]);           // members in the selected project
  const [allMembers, setAllMembers] = useState([]);     // all existing members
  ...
  const handleRemoveMember = async (userId) => {
    ...
    try {
      ...
      // Also remove tasks for that member:
      setTasksByUser(prev => {
        const next = { ...prev };
        delete next[userId];
        return next;
  ...
  // Only show members not already in project
    ...
    <Box sx={{ maxWidth: 1200, margin: "auto", mt: 5 }}>
      ...
      {!loading && selectedProjectId && (
        <Paper elevation={3} sx={{ p: 2 }}>
          ...
          {/* Add member floating button visible when members exist */}
          {members.length > 0 && (
